      ...
      run: () => state => {
        ...
        const systemctl = (action, name) => {
          ...
          if (action === 'list') {
            ...
            return available.map(s => `${s} [${running.includes(s) ? 'RUNNING' : 'STOPPED'}]`).join('\n');
          }

          if (action === 'start') {
            if (!name) return 'Usage: systemctl("start", "ServiceName")';
            if (os.listProviders().includes(name)) return `Service ${name} is already running`;

            const ServiceClass = (os.serviceClasses || {})[name];
            if (ServiceClass) {
              return os.register(ServiceClass, {name})
                .then(() => {
                  saveState();
