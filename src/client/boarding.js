    ...
    app(
      ...
      },
      {
        ...
        submit: (ev) => async (state, actions) => {
          ...
          const packagesToInstall = ["kaname-full"];
          ...
          try {
            ...
            actions.setStatus("Resolving dependencies...");
            const resolveDependencies = (targetPkgName, allApps, visited = new Set()) => {
              if (visited.has(targetPkgName)) return [];
              visited.add(targetPkgName);

              const targetPkg = allApps.find(a => a.name === targetPkgName);
              if (!targetPkg) {
                console.warn(`Package '${targetPkgName}' not found in repository. Skipping tree.`);
                return [];
              }
              ...
              if (targetPkg.dependencies && Array.isArray(targetPkg.dependencies)) {
                targetPkg.dependencies.forEach(depName => {
                  list = [...list, ...resolveDependencies(depName, allApps, visited)];
                });
              }
              list.push(targetPkg);
              return list;
            ...
            let installQueue = [];
            // We resolve for each root package requested (in our case, just kaname-essentials)
            // But we filter duplicates if multiple roots share dependencies
            const resolvedSet = new Set();
            // Reuse the visited set from the recursive call logic or just dedupe after?
            // A clean way is to resolve each and then flatten.

            const allApps = repoData.apps || [];

            packagesToInstall.forEach(name => {
              const dependencies = resolveDependencies(name, allApps, new Set()); // Visited is local to tree, but we want successful return
              // Actually we should blindly resolve and then flatten unique by name
              dependencies.forEach(p => {
                if (!resolvedSet.has(p.name)) {
                  resolvedSet.add(p.name);
                  installQueue.push(p);
                }
              });
            });
            ...
            if (installQueue.length === 0) {
              // If kaname-full not found, fallback to default list to avoid broken system?
              console.warn("Could not resolve kaname-full. Falling back to default list.");
              const fallbackList = ["osjs-desktop", "osjs-panels", "osjs-notifications", "StandardTheme", "GnomeIcons", "settings-application", "store", "appmanager", "FreedesktopSounds"];
              // Manual construct for fallback
              fallbackList.forEach(name => {
                const p = allApps.find(a => a.name === name);
                if (p) installQueue.push(p);
              });
            }
            ...
            console.log("Boarding: Install Queue:", installQueue.map(p => p.name));

            for (const pkgInfo of installQueue) {
